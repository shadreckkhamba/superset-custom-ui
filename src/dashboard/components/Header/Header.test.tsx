...
test('should move slideshow into the actions dropdown', async () => {
  window.history.pushState({}, 'Test page', '/dashboard');
  setup();

  expect(screen.queryByTestId('slideshow-button')).not.toBeInTheDocument();

  await openActionsDropdown();
  expect(screen.getByText('Enter slideshow')).toBeInTheDocument();
});

test('should toggle slideshow from the actions dropdown', async () => {
  window.history.pushState({}, 'Test page', '/dashboard');
  setup();

  await openActionsDropdown();
  userEvent.click(screen.getByText('Enter slideshow'));
  expect(screen.getByText('Clinical Service Monitoring')).toBeInTheDocument();

  await openActionsDropdown();
  userEvent.click(screen.getByText('Exit slideshow'));
  expect(
    screen.queryByText('Clinical Service Monitoring'),
